...
def _sum_to_shape(grad: np.ndarray, shape: tuple[int, ...]) -> np.ndarray:
    ...
    The gradient for B must be summed along axis 0 and axis 2, keeping
    the dimensions that were broadcast (shape (1, 3, 1)).
...
class Tensor:
    ...
    _prev : set[Tensor]
        Parent tensors that produced this tensor (only those that require gradients).
    ...
    def __init__(
        ...
        _children
            Parent tensors used to construct this tensor. Only those with
            requires_grad=True will be stored for the backward pass.

        requires_grad
            If True, gradients will be accumulated during backward.
        """
        ...
        # Store only parents that require gradients to keep the graph lean.
        self._prev: set[Tensor] = {c for c in _children if c.requires_grad}
    ...
    def backward(self) -> None:
        ...
        # Seed the gradient of the scalar output
        self.grad.fill(1.0)
