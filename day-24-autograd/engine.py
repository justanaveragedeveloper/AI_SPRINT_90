...
Implements a scalar `Value` node that builds a dynamic computational graph
(DAG) with reverse‑mode autograd. Supports addition, multiplication,
power, tanh, ReLU, exp, log, negation, and division.
"""
...
class Value:
    ...
    def __neg__(self) -> Value:
        """Unary negation."""
        result = Value(-self.value, (self,), "neg")
        result._backward_fn = self._make_neg_backward_fn(result)
        return result
    ...
    def __truediv__(self, other: Value) -> Value:
        """True division via multiplication by reciprocal."""
        return self * (other**-1)
    ...
    def exp(self) -> Value:
        ...
        result = Value(math.exp(self.value), (self,), "exp")
        result._backward_fn = self._make_exp_backward_fn(result)
        return result
    ...
    def log(self) -> Value:
        ...
        result = Value(math.log(self.value), (self,), "log")
        result._backward_fn = self._make_log_backward_fn(result)
        return result
    ...
    def _make_neg_backward_fn(self, result: Value) -> Callable[[], None]:
        def _backward() -> None:
            self.gradient += -result.gradient

        return _backward

    def _make_exp_backward_fn(self, result: Value) -> Callable[[], None]:
        def _backward() -> None:
            self.gradient += result.value * result.gradient

        return _backward

    def _make_log_backward_fn(self, result: Value) -> Callable[[], None]:
        def _backward() -> None:
            self.gradient += (1.0 / self.value) * result.gradient
